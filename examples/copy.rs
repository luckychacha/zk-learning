...
fn main() {
    ...
    let cfg = CircomConfig::<Bn254>::new(
        "test_circuits/copy_20/Copy_js/Copy.wasm",
        "test_circuits/copy_20/Copy.r1cs",
    )
    .unwrap();

    // Insert our secret inputs as key value pairs. We insert a single input, namely the input to the hash function.
    let mut builder = CircomBuilder::new(cfg);
    for (_i, value) in (1..=20).enumerate() {
        // builder.push_input(&format!("in[{}]", i), *value);
        builder.push_input("in", 100000);
    }
    // builder.push_input("in1", 1);
    // builder.push_input("in2", 0);
    // builder.push_input("in", 1);
    // builder.push_input("in", 0);
    // builder.push_input("in", 0);
    // builder.push_input("in", 0);
